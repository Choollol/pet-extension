...
1 - Add new variable to PetInfo struct
2 - Write property inside generatePetInfo()
3 - Add property to SinglePetInfo in generateSpriteInfoTypes()
4 - Update petInfo in main()
*/
...
int main() {
    ...
    generatePetInfo(outputDir, "pet-data", "ts", petInfo);
}
